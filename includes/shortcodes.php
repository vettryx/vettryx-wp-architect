...
class Vettryx_WP_Architect_Shortcodes {
    ...
    public function __construct() {
        // Registra os shortcodes durante a inicialização do WP
        ...
        foreach ($entities as $e) {
            ...
            if (!empty($e['fields'])) {
                foreach ($e['fields'] as $field) {
                    ...
                    // Padrão: [vtx_{slug-cpt}_{id-campo}]
            ...
            // 2. Gera Shortcode para Categorias: [vtx_{slug-cpt}_categorias]
            if (!empty($e['cat_slug'])) {
                $cat_tax = $cpt_slug . '_category';
                add_shortcode("vtx_{$cpt_slug}_categorias", function() use ($cat_tax) {
                    return Vettryx_WP_Architect_Shortcodes::format_taxonomy($cat_tax, 'category');
                });
            }

            // 3. Gera Shortcode para Tags: [vtx_{slug-cpt}_tags]
            if (!empty($e['tag_slug'])) {
                $tag_tax = $cpt_slug . '_tag';
                add_shortcode("vtx_{$cpt_slug}_tags", function() use ($tag_tax) {
                    return Vettryx_WP_Architect_Shortcodes::format_taxonomy($tag_tax, 'tag');
                });
            }
        }
    }
    ...
    public static function format_taxonomy($taxonomy, $type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($type === 'category') {
                ...
                // Formato de "Etiqueta" para as Tags
